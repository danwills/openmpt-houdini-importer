...
static void show_help( textout & log, bool longhelp = false, bool man_version = false, const mpt::ustring & message = mpt::ustring() ) {
	{
		...
		log << MPT_USTRING("     --pattern-time-log     Generate pattern time log (EXPERIMENTAL) [default: ") << commandlineflags().pattern_time_log << MPT_USTRING("]") << lf;
		log << lf;
...
void render_loop( commandlineflags & flags, Tmod & mod, double & duration, textout & log, write_buffers_interface & audio_stream ) {
	...
	meter_type meter;
	mpt::ustring pattern_str;
	bool pat_new = false;
	
	bool multiline = flags.show_ui;
	...
	while ( true ) {
		...
		if ( multiline ) {
			...
			if ( flags.show_meters && !flags.pattern_time_log ) {
				if ( narrow ) {
			...
			if ( flags.show_pattern ) {
				...
				if ( flags.pattern_time_log ) {
					// Always want the whole thing for pattern_time_log, with some space is nice too
					width = 16;
					// TODO: keep track of the last full-pattern-string (between '>' and ':') and only print
					// when it changes.
					// Can de-duplicate in post for now.. but this would allow update to be set really
					// small but not create a crapton of lines.
				}
				
				// TODO: When in pattern-time-log mode, check whether this line has been printed before or not
				// (according to the pattern string) and only print it when not already printed.
				// This should allow us to use very small update values but not get quite-as-enormous logs!
				...
				for ( std::int32_t line = 0; line < pattern_lines; ++line ) {
					...
					// Makes it all go on one line for pattern-log mode:
					if (!flags.pattern_time_log) {
						log << lf;
					}
					...
					if ( flags.pattern_time_log ) {
						if ( pat_new ) {
							log << MPT_USTRING("Pos: ") << seconds_to_string( mod.get_position_seconds() ) << MPT_USTRING(" / ") << seconds_to_string( duration ) << MPT_USTRING("   ") << lf;
						}
					}
				}
			}
			
			if ( flags.show_ui && !flags.pattern_time_log ) {
				if ( narrow ) {
			...
			if ( flags.show_details  && !flags.pattern_time_log ) {
...
static void parse_openmpt123( commandlineflags & flags, const std::vector<mpt::ustring> & args, concat_stream<mpt::ustring> & log ) {
	...
	for ( auto i = args.begin(); i != args.end(); ++i ) {
		...
		if ( files_only ) {
			...
			} else if ( arg == MPT_USTRING("--pattern-time-log") ) {
				flags.pattern_time_log = true;
